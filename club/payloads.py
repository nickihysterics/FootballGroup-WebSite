...
def serialize_profile(profile):
    ...
    assign_localized_field(
        payload, profile, "name", "Футбольная команда Газпром", "Gazprom Football Club"
    )
    ...
    assign_localized_field(
        payload, profile, "hero_badge", "Газпром футбольная программа", "Gazprom football program"
    )
    return payload
...
def serialize_match(match, request=None):
    ...
    payload = {
        "status": match.status,
        "status_label": MATCH_STATUS_LABELS.get(match.status, {}).get(
            "ru", match.get_status_display()
        ),
        "status_label_ru": MATCH_STATUS_LABELS.get(match.status, {}).get(
            "ru", match.get_status_display()
        ),
        "status_label_en": MATCH_STATUS_LABELS.get(match.status, {}).get(
            "en", match.get_status_display()
        ),
...
def _build_home_payload(request=None):
    ...
    featured_players = list(
        Player.objects.filter(featured=True).order_by("sort_order", "number")[:4]
    )
    ...
    featured_match = (
        Match.objects.filter(start_at__gte=now).order_by("-featured", "start_at").first()
    )
    ...
    return {
        ...
        "hero": {
            ...
            "ticker_news": [
                serialize_news(post, request)
                for post in NewsPost.objects.order_by("-published_at")[:4]
            ],
        ...
        "groups": [
            {
                ...
                "label": label,
                "players": [
                    serialize_player(player, request)
...
def _build_matches_payload(request=None):
    ...
    if not featured_match:
        featured_match = (
            Match.objects.filter(featured=True).order_by("-start_at").first()
            or Match.objects.order_by("-start_at").first()
        )
...
def _build_contacts_payload(request=None):
    profile = get_club_profile()
    recent_matches = list(
        Match.objects.filter(status=Match.Status.FINISHED).order_by("-start_at")[:3]
    )
    channels = []
