...
def localize_competition(raw: str):
    value = raw
...
def parse_schedule_datetime(month_label: str, day_label: str, time_label: str):
    ...
    if month is None or ":" not in time_label:
        return None
    hour, minute = map(int, time_label.split(":"))
    return datetime(year, month, day, hour, minute, tzinfo=MOSCOW_TZ).isoformat()
        ...
        players.append(
            {
                ...
                "position": current_position or "MF",
            }
        )
    return players
...
def parse_player_details(player_stub: dict[str, object], index: int):
    ...
    image_match = re.search(r"<div class=\"player-short-bio__image\">\s*<img src=\"([^\"]+)\"", page, re.S)
    rows = {
    ...
    name = clean_text(name_match.group(1)) if name_match else player_stub["full_name"]
    number = parse_int(number_match.group(1) if number_match else None) or player_stub["number"]
...
def parse_news(limit: int = 6):
    ...
    for url, title in article_links:
        ...
        if date_matches:
            published_at = datetime.strptime(date_matches[-1][0], "%Y-%m-%d").replace(tzinfo=MOSCOW_TZ).isoformat()
        excerpt = clean_text(excerpt_match.group(1)) if excerpt_match else title
        items.append(
            {
                "title": title,
                "excerpt": excerpt[:255],
                "body": "Официальный материал «Зенита». Полную публикацию смотрите в источнике.",
                "published_at": published_at,
                "featured": len(items) < 3,
...
def parse_matches():
            ...
            parsed.append(
                {
                    ...
                    "summary": (
                        f"{competition} · {venue} · счёт {score_for}:{score_against}."
                        if is_finished
                        else f"Предстоящий матч «Зенита» в турнире {competition}."
                    ),
    ...
    parsed.sort(key=lambda item: item["start_at"], reverse=True)
    upcoming = sorted([item for item in parsed if item["status"] == "upcoming"], key=lambda item: item["start_at"])
    recent = sorted([item for item in parsed if item["status"] == "finished"], key=lambda item: item["start_at"], reverse=True)
    if upcoming:
...
def main():
    players = [parse_player_details(stub, index) for index, stub in enumerate(parse_squad(), start=1)]
    news = parse_news()
    ...
    out = Path(__file__).resolve().parents[1] / "data" / "zenit_reference.json"
    out.parent.mkdir(parents=True, exist_ok=True)
    out.write_text(json.dumps(snapshot, ensure_ascii=False, indent=2), encoding="utf-8")
    print(f"Snapshot saved to {out}")
    print(f"Players: {len(players)} | News: {len(news)} | Matches: {len(matches)}")
