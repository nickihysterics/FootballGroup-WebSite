import { resolveMatchState } from "./matchState.js";
export { resolveMatchState } from "./matchState.js";
...
export function resolveStatusMeta(match, language = "ru") {
  ...
  if (state === "archived") return { label: translate(language, "match.status.archived"), tone: "neutral", icon: Clock3 };
...
export function normalizeMatch(match, index, language = "ru") {
  ...
  return {
    ...normalized,
    summaryText: state === "archived" ? translate(language, "match.status.archived") : summary || buildSummaryText(normalized, language),
