...
export default function TeamPage() {
  ...
  const visibleGroups = groups.filter((group) => group.players.length);
  ...
  return (
    <>
      <Reveal y={22} duration={0.5}>
        <section className="page-banner">
          <div className="page-banner__copy card">
            ...
            <p>
              Крупные портреты, статистика по матчам и короткие игровые профили
              для каждого футболиста.
            </p>
          ...
          {captain ? (
            <article className="captain-hero">
              <div className="captain-hero__visual">
                {captain.photo_url ? (
                  <img src={captain.photo_url} alt={captain.full_name} />
                ) : (
                  <span>{captain.initials}</span>
                )}
              </div>
      ...
      {visibleGroups.map((group, index) => (
        ...
        >
          <section className="section-block roster-section">
              ...
              {group.players.map((player) => (
                <article key={player.id} className="roster-card">
                  ...
                  <div className="roster-card__facts">
                    <span>
                      {player.birth_date_label || "Дата рождения не указана"}
                    </span>
                    <span>
                    ...
                    <span>
                      {player.previous_club
                        ? `Предыдущий клуб: ${player.previous_club}`
                        : "Основной состав"}
                    </span>
                  </div>
                  ...
                  <div className="roster-card__stats">
                    <div>
                      <span>Рост</span>
                      <strong>
                        {player.height_cm ? `${player.height_cm} см` : "—"}
                      </strong>
                    </div>
                    <div>
                      <span>Вес</span>
                      <strong>
                        {player.weight_kg ? `${player.weight_kg} кг` : "—"}
                      </strong>
                    </div>
                    <div>
                      <span>Матчи</span>
